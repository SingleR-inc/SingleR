...
We provide plotScoreDistribution to help in determining whether cutoffs might need adjustment.

The main check is a per-label check where low outliers within each label are removed, based on whether they are more than `nmads` median absolute deviations (MADs) below the median score for that label.  We additionally provide inputs for setting two additional pruning cutoffs, but these are set to 0 by default:

- a minimum delta between a cell's max score and the median of its scores, `min.diff.med`
- a minimum difference between a cells best fine-tuning score and its next best fine-tuning score, `min.diff.next`

```{r}
# Showing raw scores (default):
plotScoreDistribution(pred, show = "scores", ncol = 3)

# Showing deltas versus the median scores for a given label:
  # This is useful for checking/adjusting nmads and min.diff.med
plotScoreDistribution(pred, show = "delta.med", ncol = 3, show.nmads = 3, show.min.diff = 0.1)

# Showing deltas versus the median scores for a given label:
  # This is useful for checking/adjusting min.diff.next
plotScoreDistribution(pred, show = "delta.next", ncol = 3, show.min.diff = 0.05)
```
