...
date: "Revised: November 6th, 2019"
output:
...
Ideally, each cell (i.e., column) should have one score that is obviously larger than the rest, indicating that it is unambiguously assigned to a single label.
...
If this is not the case, some skepticism towards the quality of the assignments is warranted.
...
# Combining predictions from different references

In many cases, running the test set against multiple reference datasets may be useful to increase granularity among certain cell types or to look for concordance across multiple references. 
The `combineResults` function will collate multiple predictions together for convenience:
...
# Restrict to common genes between test and reference data:
library(scater)
common.hpca <- intersect(rownames(hESCs), rownames(hpca.se))
hpca.se <- hpca.se[common.hpca,]
hESCs.hpca <- hESCs[common.hpca,]
hESCs.hpca <- logNormCounts(hESCs.hpca)

common.bp <- intersect(rownames(hESCs), rownames(bp.se))
bp.se <- bp.se[common.bp,]
hESCs.bp <- hESCs[common.bp,]
hESCs.bp <- logNormCounts(hESCs.bp)

pred.hpca <- SingleR(test = hESCs.hpca, ref = hpca.se, labels = hpca.se$label.main)
pred.bp <- SingleR(test = hESCs.bp, ref = bp.se, labels = bp.se$label.main)

combined.preds <- combineResults(list(pred.hpca, pred.bp))
