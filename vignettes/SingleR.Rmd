...
For more details see the vignette dedicated to the `r Biocpkg("SingleR", vignette = "datasets.html", label="reference datasets")`.

Each reference dataset is obtained with a specific function: `HumanPrimaryCellAtlasData()`, `BlueprintEncodeData()`, `ImmGenData()`, `MouseRNAseqData()`.
...
We subset to genes that are present in both the scRNA-seq data to be annotated and the reference data set.
We then compute log-normalized expression values for the hESC data set.
...
The "main" labels tend to be less fine-grained than the "fine" labels, e.g. instead of specifying different subsets of T cells, the main labels will lump them all together under the broader label "T cells".
For more details about the individual cell type labels available for the different reference data sets, see the vignette `r Biocpkg("SingleR", vignette = "datasets.html", label="reference datasets")`.

For our example, we will use the more specific fine-grained cell type labels.

```{r}
pred.hpca <- SingleR(test = hESCs, ref = hpca.se, labels = hpca.se$label.fine)
