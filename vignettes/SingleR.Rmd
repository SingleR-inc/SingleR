...
We can prune out potentially poor-qality assignments, and assignments in which SingleR could not make a strong determination between multiple labels, with the `pruneScores()` function.

```{r}
# To determine what labels might need pruning, we can run.
pruneScores(pred)
```

To create a vector of labels where the pruned labels are set to `NA`, we can use the below code.  In fact, code like this is run by default when `SingleR()` is run.

```{r}
pred$pruned.labels <- pred$labels
pred$pruned.labels[pruneScores(pred)] <- NA_character_
```

Default parameters for pruning may not be perfect for every dataset, so we provide a few visualization functions for assessing when cutoffs might need adjustment.

The first cutoff is a per cell check that the maximum score associated with that cell (prior to fine-tuning) is at least minimum level above the median value of scores for that cell.  The cutoff is controlled by `pruneScores()` parameter `min.diff.med` and the function `plotScoresSingleCell()` aids visulaization of whether this parameter is set nicely.
...
We can see that here, the maximal score was less than 0.05 greater than the median score of that cell.

We would change the min.diff.med parameter if...


The second cutoff...

And a set of per label functions can help determine if this parameter needs to be adjusted, `plotScoresSingleLabel()` and `plotScoresMultiLabels()`

```{r}
plotScoresMultiLabels(pred, dots.on.top = TRUE)
plotScoresSingleLabel(pred, label = "beta", dots.on.top = TRUE)
plotScoresSingleLabel(pred, label = "alpha", dots.on.top = TRUE)
```
