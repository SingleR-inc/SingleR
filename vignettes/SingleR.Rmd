...
We examine the main cell types contained within each reference:

```{r}
unique(HumanPrimaryCellAtlasData()$label.main)
unique(BlueprintEncodeData()$label.main)
unique(DatabaseImmuneCellExpressionData()$label.main)
unique(NovershternHematopoieticData()$label.main)
unique(MonacoImmuneData()$label.main)
unique(ImmGenData()$label.main)
unique(MouseRNAseqData()$label.main)
```
