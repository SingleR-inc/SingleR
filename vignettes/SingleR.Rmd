...
see `?getReferenceDataset` for more information. 
...
This can then be used for annotation of a scRNA-seq dataset.
To illustrate, we will use a hESC dataset from `r Biocpkg("scRNAseq")` [@lamanno2016molecular].
Again, we will subset this test dataset for the sake of speed.
...
We subset to the common genes and we generate log-normalized expression values as previously described.
...
```{r}
pred.hpca <- SingleR(test=hESCs, ref=hpca$data, labels=hpca$types)
