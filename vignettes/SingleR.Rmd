...
`r Biocpkg("SingleR")` comes pre-packaged with a number of training datasets generated from bulk RNA-seq of pure cell types.
This makes use of Immgen, Blueprint+Encode, the Human Primary Cell Atlas and other resources.
Each reference dataset is obtained from `r Biocpkg("ExperimentHub")` by using the relevant command:

```{r, eval=FALSE}
hpca <- HPCATrainingSet()
```

This can then be directly used for annotation of our scRNA-seq dataset.
In this case, we will use the precomputed marker genes in `de.genes` though users are free to define their own markers.

```{r, eval=FALSE}
pred.hpca <- SingleR(test=sceG, training=hpca$data, 
    labels=hpca$labels, genes=hpca$de.genes)
```
