...
Dvir Aran's original `r Githubpkg("dviraran/SingleR")` github repository contains Robjects with normalized expression values from reference data sets such as those collected by Blueprint & Encode, the Human Primary Cell Atlas, and ImmGen.
...
dataset <- "blueprint_encode" # Human Primary Cell Atlas
...
```{r}
table(duplicated(colnames(normcounts)))
```

Uniquifying the colnames:

```{r}
colnames(normcounts) <- paste(colnames(normcounts), 1:ncol(normcounts), sep = ".")
```
