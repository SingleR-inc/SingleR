    ...
    silent = FALSE, ..., grid.vars = list())
{
    ...
    for (i in seq_along(plots)) {
        ...
        scores.title <- "Scores"
        if (is.combined) {
            scores.title <- paste0(scores.title, " (", score.name, ")")
        }
        ...
        labels.title <- "Labels"
        if (is.combined) {
            labels.title <- paste0(labels.title, " (", call.name, ")")
        }
...
.plot_score_heatmap <- function(
    ...
    clusters, order.by, show.labels, show.pruned,
